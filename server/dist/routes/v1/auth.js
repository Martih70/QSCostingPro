...
import { z } from 'zod';
import { registerUser, loginUser, refreshAccessToken, logoutUser, userToPublic, verifyUserEmail, sendPasswordReset, resetPassword, deleteUser, } from '../../services/authService.js';
 ...
 */
router.post('/send-verification-email', verifyAuth, async (req, res) => {
    try {
        if (!req.user) {
            res.status(401).json({
                success: false,
