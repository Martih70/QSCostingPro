...
import { ensureSectionsFromImport, rebuildLibrary, } from '../../services/boqLibraryService.js';
 ...
 */
router.post('/import', verifyAuth, authorize('admin'), upload.single('file'), async (req, res) => {
    let filePath = null;
    try {
        ...
        `);
            const importResult = importStmt.run(boqData.importName, boqData.sections.length, boqData.totalItems, req.user.userId);
            const boqImportId = importResult.lastInsertRowid;
            // Get the next available section number globally (not just per import)
            const maxSectionStmt = db.prepare('SELECT MAX(section_number) as maxNum FROM boq_library_sections');
            const maxResult = maxSectionStmt.get();
            ...
            const sectionInputs = boqData.sections.map((section, idx) => ({
            ...
            const getSectionIdStmt = db.prepare('SELECT id FROM boq_library_sections WHERE section_number = ?');
        ...
        `);
            for (let idx = 0; idx < boqData.sections.length; idx++) {
                ...
                const sectionRow = getSectionIdStmt.get(sectionNumber);
                if (!sectionRow) {
                ...
                for (const item of section.items) {
                    ...
                    insertItemStmt.run(sectionId, item.itemNumber, item.description, item.quantity, item.unit, item.rate, item.notes || null, 1, // is_active
                    req.user.userId);
                }
            }
            return boqImportId;
        ...
        logger.info(`BoQ imported to library: ${boqData.totalItems} items, ${boqData.sections.length} sections by ${req.user?.username}`);
        res.status(201).json({
            success: true,
            data: {
 ...
 */
router.get('/summary', verifyAuth, (_req, res) => {
    try {
        const db = getDatabase();
 ...
 */
router.get('/imports', verifyAuth, authorize('admin'), (_req, res) => {
    try {
        const db = getDatabase();
...
//# sourceMappingURL=boqRepository.js.map