 ...
 */
router.post('/', verifyAuth, validate(createTemplateSchema), (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            res.status(401).json({
 ...
 */
router.put('/:id', verifyAuth, validate(updateTemplateSchema), (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            res.status(401).json({
 ...
 */
router.post('/apply/:templateId/to/:projectId', verifyAuth, (req, res) => {
    try {
        const user = req.user;
        if (!user) {
            res.status(401).json({
...
//# sourceMappingURL=estimateTemplates.js.map