...
import { costItemsRepository, subElementsRepository, unitsRepository, } from '../../repositories/costRepository.js';
...
router.get('/', verifyAuth, (req, res) => {
    try {
        ...
        let databaseType = req.query.database_type || (user.is_witness ? 'witness' : 'standard_uk');
        ...
        const costItems = Object.keys(filters).length > 1
 ...
 */
router.get('/sub-element/:subElementId', verifyAuth, (req, res) => {
    try {
 ...
 */
router.post('/', verifyAuth, authorize('admin', 'estimator'), validate(createCostItemSchema), (req, res) => {
    try {
        const { sub_element_id, code, description, unit_id, material_cost, management_cost, contractor_cost, is_contractor_required, volunteer_hours_estimated, waste_factor, currency, price_date, region, } = req.body;
        ...
        logger.info(`Cost item created: ${costItem.code} (${unit.code}) by user ${req.user?.username}`);
        res.status(201).json({
            success: true,
            data: costItem,
        });
 ...
 * Requires: admin or estimator role
 */
router.put('/:id', verifyAuth, authorize('admin', 'estimator'), validate(updateCostItemSchema), (req, res) => {
    try {
        const { id } = req.params;
        const costItemId = parseInt(id, 10);
        if (isNaN(costItemId)) {
        ...
        const { description, material_cost, management_cost, contractor_cost, is_contractor_required, volunteer_hours_estimated, waste_factor, region, price_date, } = req.body;
...
//# sourceMappingURL=costItems.js.map