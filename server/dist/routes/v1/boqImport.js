 ...
 */
router.post('/:projectId/boq-import/preview', verifyAuth, authorize('admin', 'estimator'), upload.single('file'), async (req, res) => {
    let filePath = null;
    try {
 ...
 */
router.post('/:projectId/boq-import', verifyAuth, authorize('admin', 'estimator'), upload.single('file'), async (req, res) => {
    let filePath = null;
    try {
        const { projectId } = req.params;
        const projectIdNum = parseInt(projectId, 10);
        ...
        `);
            const importResult = importStmt.run(projectIdNum, boqData.importName, boqData.totalItems, boqData.grandTotal, req.user.userId);
            const boqImportId = importResult.lastInsertRowid;
            // Insert line items
        ...
        `);
            ...
            for (const section of boqData.sections) {
                ...
                for (const item of section.items) {
                    ...
                    const isHeader = item.itemNumber === `${section.sectionNumber}.0` ||
                        item.itemNumber === `${section.sectionNumber}.0.0` ||
                        item.itemNumber === section.sectionNumber;
                    if (isHeader) {
                        continue;
                    }
                    insertItemStmt.run(projectIdNum, item.quantity, item.rate, item.notes || null, item.amount, req.user.userId, 1, // version_number
                    1, // is_active
                    item.description, item.unit, 1, // Default category ID (should be made configurable)
                    item.itemNumber, sectionCounter, section.sectionTitle, boqImportId);
                }
            }
            return boqImportId;
        });
        const boqImportId = transaction();
        logger.info(`BoQ imported for project ${projectIdNum}: ${boqData.totalItems} items, £${boqData.grandTotal.toFixed(2)} by user ${req.user?.username}`);
        res.status(201).json({
            success: true,
            data: {
 ...
 */
router.get('/:projectId/boq-imports', verifyAuth, (req, res) => {
    try {
        const { projectId } = req.params;
        const projectIdNum = parseInt(projectId, 10);
        if (isNaN(projectIdNum)) {
 ...
 */
router.delete('/:projectId/boq-imports/:importId', verifyAuth, authorize('admin', 'estimator'), (req, res) => {
    try {
        ...
        const existingImport = importStmt.get(importIdNum, projectIdNum);
        if (!existingImport) {
            res.status(404).json({
                success: false,
        ...
        transaction();
        logger.info(`BoQ import ${importIdNum} deleted from project ${projectIdNum} by user ${req.user?.username}`);
        res.json({
            success: true,
...
//# sourceMappingURL=boqImport.js.map