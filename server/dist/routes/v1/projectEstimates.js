...
}).refine((data) => data.cost_item_id || (data.custom_description && data.custom_unit_rate !== undefined && data.custom_unit), { message: 'Either cost_item_id or all custom fields (description, unit, rate) must be provided' });
...
router.post('/:projectId/estimates/:estimateId/cost-components', verifyAuth, authorize('admin', 'estimator'), (req, res) => {
    try {
        ...
        const updatedComponent = costComponentsRepository.getByEstimateId(estimateIdNum).find(c => c.component_type === component_type && c.is_active);
        res.status(201).json({
            success: true,
 ...
 */
router.get('/:projectId/estimates/export-boq-pdf', verifyAuth, async (req, res) => {
    try {
        const { projectId } = req.params;
        const projectIdNum = parseInt(projectId, 10);
        ...
        if (boqImportId) {
            const boqImportIdNum = parseInt(boqImportId, 10);
        ...
        const boqSections = Array.from(sections.values()).sort((a, b) => parseInt(a.sectionNumber) - parseInt(b.sectionNumber));
        ...
        // Set response headers
        res.setHeader('Content-Type', 'application/pdf');
        res.setHeader('Content-Disposition', `attachment; filename="BoQ_${project.name.replace(/[^a-z0-9]/gi, '_')}_${new Date()
            .toISOString()
            .split('T')[0]}.pdf"`);
        // Pipe to response
        doc.pipe(res);
