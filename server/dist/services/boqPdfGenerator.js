...
import { calculatePagination, formatCurrency } from './boqImportService.js';
/**
 ...
 */
export function generateBoQPDF(sections, options) {
    ...
    for (const section of paginatedSections) {
        for (const page of section.pages) {
            doc.addPage();
            generateSectionPage(doc, section, page, options, pageLayout);
        }
 ...
 */
function generateTitlePage(doc, options, sections) {
    ...
    doc
        ...
        .text(`Date: ${(options.generatedDate || new Date()).toLocaleDateString('en-GB')}`, {
        align: 'center',
    });
}
/**
 ...
 */
function generateSectionPage(doc, section, page, _options, layout) {
    const { pageWidth, marginLeft, marginRight, marginTop, marginBottom, columnWidths } = layout;
    // Header
    doc
        .fontSize(11)
        .font('Helvetica-Bold')
        .text(`${section.sectionNumber}. ${section.sectionTitle}`, marginLeft, marginTop, { width: pageWidth - marginLeft - marginRight });
    ...
    drawTableCell(doc, 'Description', marginLeft + columnWidths.itemNumber + 5, headerY + 5, columnWidths.description);
    drawTableCell(doc, 'Qty', marginLeft + columnWidths.itemNumber + columnWidths.description + 5, headerY + 5, columnWidths.quantity);
    drawTableCell(doc, 'Unit', marginLeft +
        columnWidths.itemNumber +
        columnWidths.description +
        columnWidths.quantity +
        5, headerY + 5, columnWidths.unit);
    drawTableCell(doc, 'Rate (£)', marginLeft +
        columnWidths.itemNumber +
        columnWidths.description +
        columnWidths.quantity +
        columnWidths.unit +
        5, headerY + 5, columnWidths.rate);
    drawTableCell(doc, 'Amount (£)', marginLeft +
        columnWidths.itemNumber +
        columnWidths.description +
        columnWidths.quantity +
        columnWidths.unit +
        columnWidths.rate +
        5, headerY + 5, columnWidths.amount);
    ...
    for (const item of page.items) {
        ...
        drawTableCell(doc, item.description, marginLeft + columnWidths.itemNumber + 5, currentY, columnWidths.description, 3);
        drawTableCell(doc, item.quantity.toFixed(2), marginLeft + columnWidths.itemNumber + columnWidths.description + 5, currentY, columnWidths.quantity, 3, 'right');
        drawTableCell(doc, item.unit, marginLeft +
            columnWidths.itemNumber +
            columnWidths.description +
            columnWidths.quantity +
            5, currentY, columnWidths.unit, 3);
        drawTableCell(doc, formatCurrency(item.rate), marginLeft +
            columnWidths.itemNumber +
            columnWidths.description +
            columnWidths.quantity +
            columnWidths.unit +
            5, currentY, columnWidths.rate, 3, 'right');
        drawTableCell(doc, formatCurrency(item.amount), marginLeft +
            columnWidths.itemNumber +
            columnWidths.description +
            columnWidths.quantity +
            columnWidths.unit +
            columnWidths.rate +
            5, currentY, columnWidths.amount, 3, 'right');
        currentY += rowHeight;
    }
    // Page subtotal
 ...
 */
function generateSectionSummary(doc, section, _options, layout) {
 ...
 */
function generateFinalSummary(doc, sections, options, layout) {
    const { pageWidth, marginLeft, marginRight, marginTop } = layout;
    // Header
    doc
        .fontSize(16)
    ...
    for (const section of sections) {
        ...
        doc.text(`${section.sectionNumber}. ${section.sectionTitle}: ${formatCurrency(sectionTotal)}`);
    }
    // Grand total
    doc.moveDown(1);
    ...
    doc
        ...
        .text(`Generated: ${(options.generatedDate || new Date()).toLocaleDateString('en-GB')} at ${(options.generatedDate || new Date()).toLocaleTimeString('en-GB')}`);
}
/**
 ...
 */
function drawTableCell(doc, text, x, y, width, lineHeight, align) {
 ...
 */
function calculateTotalPages(sections) {
...
//# sourceMappingURL=boqPdfGenerator.js.map