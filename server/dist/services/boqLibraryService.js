 ...
 */
export function ensureSectionsFromImport(db, sections, userId, _importId) {
 ...
 */
export function repaginateLibrarySection(db, sectionRow) {
  ...
  `);
    const items = itemsStmt.all(sectionId);
 ...
 */
export function regenerateLibraryCollection(db, sectionRow) {
  ...
  `);
    const pages = pagesStmt.all(sectionId);
    if (pages.length === 0) {
 ...
 */
export function regenerateLibrarySummary(db) {
 ...
 * Called after import.
 */
export function rebuildLibraryStructure(db) {
    // Fetch all sections
...
//# sourceMappingURL=boqLibraryService.js.map