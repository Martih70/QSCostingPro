...
export const estimateTemplateRepository = {
    ...
    getAll: (userId, includeShared = true) => {
        try {
    ...
    getById: (id, userId) => {
        try {
            ...
            const itemsStmt = db.prepare('SELECT * FROM template_line_items WHERE template_id = ? ORDER BY sequence_order ASC');
            const line_items = itemsStmt.all(id);
            return {
                ...template,
                line_items,
            };
    ...
    create: (userId, data) => {
      ...
      `);
            const result = insertTemplate.run(userId, data.name, data.description || null, data.template_type || 'standard', data.is_public ? 1 : 0);
      ...
      `);
            ...
            data.line_items.forEach((item, index) => {
                const itemResult = insertItem.run(templateId, index + 1, // sequence_order
                item.cost_item_id || null, item.custom_description || null, item.custom_unit_rate || null, item.custom_unit || null, item.category_id || null, item.quantity || null, item.unit_cost_override || null, item.notes || null);
                line_items.push({
    ...
    update: (id, userId, data) => {
        ...
        WHERE id = ?
      `);
            updateStmt.run(data.name !== undefined ? data.name : existingTemplate.name, data.description !== undefined ? data.description : existingTemplate.description, data.template_type !== undefined ? data.template_type : existingTemplate.template_type, data.is_public !== undefined ? (data.is_public ? 1 : 0) : existingTemplate.is_public, id);
        ...
        `);
                data.line_items.forEach((item, index) => {
                    insertItem.run(id, index + 1, item.cost_item_id || null, item.custom_description || null, item.custom_unit_rate || null, item.custom_unit || null, item.category_id || null, item.quantity || null, item.unit_cost_override || null, item.notes || null);
                });
            }
    ...
    getByType: (templateType) => {
        try {
            const db = getDatabase();
            const stmt = db.prepare('SELECT * FROM estimate_templates WHERE template_type = ? AND is_public = 1 ORDER BY created_at DESC');
            return stmt.all(templateType);
        }
        catch (error) {
...
//# sourceMappingURL=estimateTemplateRepository.js.map