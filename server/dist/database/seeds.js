...
function seedUnits(db) {
    try {
        ...
        if (count === 0) {
            const insertStmt = db.prepare('INSERT INTO units (code, name, unit_type) VALUES (?, ?, ?)');
...
function seedCategories(db) {
    try {
        ...
        const { count } = checkStmt.get();
        if (count === 0) {
            const insertStmt = db.prepare('INSERT INTO cost_categories (code, name, description, sort_order) VALUES (?, ?, ?, ?)');
...
function seedSubElements(db) {
    try {
        ...
        const categories = getCatStmt.all();
        ...
        const { count } = checkStmt.get();
        if (count === 0) {
            const insertStmt = db.prepare('INSERT INTO cost_sub_elements (category_id, code, name, description, sort_order) VALUES (?, ?, ?, ?, ?)');
    ...
    try {
      ...
      `);
            for (const item of costItems) {
                ...
                if (subId && unitId) {
                    insertStmt.run(subId, item.code, item.description, unitId, item.materialCost, item.managementCost, item.contractorCost, item.isContractor ? 1 : 0, item.volunteerHours, 'GBP', new Date().toISOString().split('T')[0], 'witness');
                }
            }
...
function seedStandardUKCosts(db) {
    try {
        ...
        if (!categoryId) {
            ...
            const catResult = catInsertStmt.run('UK-001', 'Standard UK Construction', 'Standard UK market construction costs', 100);
    ...
    `);
        for (const item of standardUKItems) {
            ...
            const unitId = unitMap.get(item.unitCode);
            if (subId && unitId) {
                costInsertStmt.run(subId, item.code, item.description, unitId, item.materialCost, item.managementCost, item.contractorCost, 1, // is_contractor_required
                'GBP', new Date().toISOString().split('T')[0], 'standard_uk');
            }
        }
