...
export function generateTokens(payload: Omit<JWTPayload, 'iat' | 'exp'>): TokenPair {
  ...
  const accessToken = jwt.sign(
    payload,
    secret,
    { expiresIn: config.jwtAccessTokenExpires } as SignOptions
  );
  const refreshToken = jwt.sign(
    payload,
    secret,
    { expiresIn: config.jwtRefreshTokenExpires } as SignOptions
  );
 ...
 * Login user
 */
export async function loginUser(req: LoginRequest): Promise<{ user: User; tokens: TokenPair }> {
  try {
    // Find user
...
export async function refreshAccessToken(
  ...
  try {
    ...
    `);
    const storedToken = stmt.get(user.id) as { token_hash: string } | undefined;
