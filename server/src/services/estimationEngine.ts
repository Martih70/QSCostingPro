...
): { material: number; management: number; contractor: number; total: number } {
 ...
 */
export function calculateProjectEstimates(projectId: number): LineItemCalculation[] {
  try {
    ...
    return rows.map((row) => {
      ...
      let material_total: number = 0;
      let management_total: number = 0;
      let contractor_total: number = 0;
      ...
      if (componentTotal !== undefined && componentTotal > 0) {
        ...
        const costs = calculateLineTotal(
          row.quantity,
          row.material_cost,
          row.management_cost,
          row.contractor_cost,
          row.waste_factor,
          row.is_contractor_required
        );
...
export function calculateCategoryTotals(
  ...
  try {
    ...
    return categories.map((category) => {
      ...
      const contractorSubtotal = categoryItems.reduce(
 ...
 */
export function calculateProjectTotal(projectId: number): ProjectEstimateTotal {
  try {
    const db = getDatabase();
    ...
    const costPerM2 =
      project.floor_area_m2 && project.floor_area_m2 > 0
 ...
 */
export function updateLineItemTotal(estimateId: number, lineTotal: number): void {
  try {
    const db = getDatabase();
 ...
 */
export function getCostPerM2Benchmark(
  categoryId: number,
  region?: string,
  buildingAge?: number,
  ...
  try {
    ...
    if (buildingAge !== undefined && buildingAge !== null) {
      let ageRange = '30+';
      if (buildingAge < 10) ageRange = '0-10';
      else if (buildingAge < 20) ageRange = '10-20';
      else if (buildingAge < 30) ageRange = '20-30';
    ...
    if (conditionRating !== undefined && conditionRating !== null) {
      let ratingRange = '4-5';
      if (conditionRating <= 2) ratingRange = '1-2';
      else if (conditionRating === 3) ratingRange = '3';
 ...
 */
export function compareToHistoricData(
  projectId: number,
  categoryId: number
): { estimated_cost_per_m2: number | null; historic_cost_per_m2: number | null; variance_percent: number | null } {
  try {
    const db = getDatabase();

    // Get project details
    const projectStmt = db.prepare(
      'SELECT floor_area_m2, region, building_age, condition_rating FROM projects WHERE id = ?'
    );
    const project = projectStmt.get(projectId) as any;
    ...
    const historicCostPerM2 = getCostPerM2Benchmark(
      categoryId,
      project.region,
      project.building_age,
      project.condition_rating
    );
 ...
 */
export function getEstimateSummary(
  projectId: number
): {
