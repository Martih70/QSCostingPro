 ...
 */
export function parseBoQCSV(csvContent: string): BoQImportData {
  try {
    ...
    if (itemNoIdx === -1 || descIdx === -1 || unitIdx === -1 || quantityIdx === -1 || rateIdx === -1 || amountIdx === -1) {
      throw new Error(
        `Missing required columns. Found: ${headers.join(', ')}. Expected: Item No., Description, Unit, Quantity, Rate (£), Amount (£). Optional: Notes`
      );
    }
    ...
    for (const [sectionNumber, sectionItems] of Array.from(sectionMap.entries()).sort()) {
      ...
      const headerItem = sectionItems.find(
        item => item.itemNumber === `${sectionNumber}.0` ||
 ...
 */
export function validateBoQData(data: BoQImportData): string[] {
  const errors: string[] = [];
 ...
 */
export function calculatePagination(sections: BoQSection[], itemsPerPage: number = 20) {
  const paginatedSections: Array<{
    sectionId: number;
    sectionNumber: string;
    sectionTitle: string;
    pages: Array<{
 ...
 */
export function extractSectionId(itemNumber: string): number {
