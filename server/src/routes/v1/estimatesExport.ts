...
router.get('/:projectId/estimates/export-pdf', verifyAuth, (req: Request, res: Response) => {
  try {
    ...
    if (groupBy === 'nrm2') {
      ...
      const stmt = db.prepare(`
        ...
        ORDER BY
          ...
          pe.id
      `)
      estimatesWithNRM2 = stmt.all(projectIdNum) as any[]
    } else {
      // Standard estimates
      estimatesWithNRM2 = projectEstimatesRepository.getAll(projectIdNum) as any[]
    }
    ...
    doc.fontSize(11)
    doc.text(
      `Subtotal: £${totals.subtotal.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )
    doc.text(
      `Contingency (${totals.contingency_percentage}%): £${totals.contingency_amount.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )

    doc.font('Helvetica-Bold').fontSize(12)
    doc.text(
      `GRAND TOTAL: £${totals.grand_total.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
    )

    if (totals.cost_per_m2) {
      doc.fontSize(10).font('Helvetica')
      doc.text(
        `Cost per m²: £${totals.cost_per_m2.toLocaleString('en-GB', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}`
      )
    }
