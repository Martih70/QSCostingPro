 ...
 */
router.get('/sections', verifyAuth, (_req: Request, res: Response) => {
  try {
 ...
 */
router.get('/sections/:sectionId', verifyAuth, (req: Request, res: Response) => {
  try {
    ...
    const sectionStmt = db.prepare(`
      ...
      WHERE id = ?
    `);
    const section = sectionStmt.get(sectionIdNum) as {
      id: number;
      section_number: number;
      section_title: string;
      items_per_page: number;
    ...
    `);
    const pages = pagesStmt.all(sectionIdNum) as Array<{
      id: number;
      page_number: number;
      total_pages: number;
      item_count: number;
    ...
    `);
    const allItems = itemsStmt.all(sectionIdNum) as Array<{
      id: number;
      item_number: string;
      description: string;
      unit: string;
...
router.post(
  ...
  (req: Request, res: Response) => {
    try {
      ...
      `);
      const libraryItems = itemsStmt.all(...item_ids) as Array<{
        id: number;
        section_id: number;
        item_number: string;
        description: string;
      ...
      const transaction = db.transaction(() => {
        ...
        for (const libraryItem of libraryItems) {
          const result = insertStmt.run(
            projectIdNum,
            null, // quantity - user will enter
            libraryItem.standard_rate,
            libraryItem.description,
            libraryItem.unit,
            null, // line_total - will calculate when quantity added
            req.user!.userId,
            1, // version_number
            1, // is_active
            libraryItem.item_number,
            libraryItem.section_id,
            1 // default category
          );
      ...
      logger.info(
        `Copied ${copiedEstimates.length} items from library to project ${projectIdNum} by ${req.user?.username}`
      );
  ...
  (req: Request, res: Response) => {
    try {
      ...
      const itemsStmt = db.prepare(`
        ...
        ORDER BY item_number ASC, id ASC
      `);
      const sectionItems = itemsStmt.all(section_id) as Array<{
        id: number;
        item_number: string;
        description: string;
        unit: string;
      ...
      logger.info(
        `Copied section ${section_id} (${copiedEstimates.length} items) to project ${projectIdNum} by ${req.user?.username}`
      );
